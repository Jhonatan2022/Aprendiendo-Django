...
def index(request):
    ...
    return render(request, "index.html", {"titulo": tittle, "nameuser": nameuser})
...
def tasks(request):
    ...
    return render(request, "tasks/tasks.html", {"tasks": tarea})
...
def completed_tasks(request):
    ...
    tarea = task.objects.filter(
        user=request.user, datecompleted__isnull=False
    ).order_by(
        "-datecompleted"
    )  # Usamos el método order_by para ordenar los datos de la tabla task
...
def projects(request):
    ...
    return render(
        request, "projects/projects.html", {"projects": datosprojects}
...
def create_project(request):
    ...
    if request.method == "GET":
        return render(
            request, "projects/create_project.html", {"form": Createproject()}
        )
